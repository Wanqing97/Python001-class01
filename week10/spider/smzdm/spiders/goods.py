...
from smzdm.items import SpiderItem
...
class GoodsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for url_xpath in phone_urls:
            ...
            yield scrapy.Request(url = url, dont_filter = True, meta = {'phone_id': phone_id}, callback = self.phone_page)

        next_page = sel.xpath('//li[(@class="page-turn  next-page")]//a//@href')
        if next_page:
            yield scrapy.Request(''.join(next_page.extract()).strip(), dont_filter = True, callback = self.parse)
    ...
    def phone_page(self, response):
        ...
        yield scrapy.Request(response.url, meta = {'phone_info': phone_info}, callback = self.comments_page, dont_filter = True)
    ...
    def comments_page(self, response):
        ...
        if next_page:
            ...
            yield scrapy.Request(url = url, meta = {'phone_info': phone_info}, callback = self.comments_page, dont_filter = True)